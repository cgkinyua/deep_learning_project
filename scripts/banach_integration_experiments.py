...
This script integrates Banach space ideals into the iterative refinement module
of a refined autoencoder by (1) applying spectral normalization to constrain
the Lipschitz constant and (2) adding a contraction regularization loss term.
It supports two training modes:
    - Standard mode: iterative refinement without contraction loss.
    - Contraction mode: adds a regularization term that penalizes large differences
      between successive refinement outputs.

The script also runs experiments with different lambda_contraction values to
compare performance.

Usage:
    python banach_integration_experiments.py --epochs 10 --use_contraction True --lambda_contraction 0.1 --refine_iter 5 --refine_scale 0.7

Arguments:
    --epochs             : Number of training epochs (default: 10)
    --use_contraction    : Flag to enable contraction regularization (default: False)
    --lambda_contraction : Weight of the contraction regularization term (default: 0.1)
    --refine_iter        : Number of iterative refinement steps (default: 5)
    --refine_scale       : Refinement scale factor (default: 0.7)
"""
...
class Autoencoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(3, 16, kernel_size=3, stride=2, padding=1),  # Output: (B,16,16,16)
            nn.ReLU(),
            nn.Conv2d(16, 32, kernel_size=3, stride=2, padding=1), # Output: (B,32,8,8)
            nn.ReLU(),
            nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1), # Output: (B,64,4,4)
            nn.ReLU()
        )
...
class RefinedAutoencoder(nn.Module):
    ...
    def __init__(self, base_autoencoder, num_iterations=5, refinement_scale=0.7):
        ...
        # Apply spectral normalization to each conv layer in the refinement module
...
def train_model(model, trainloader, valloader, device, num_epochs=10, noise_factor=0.1):
    """Standard training function without contraction loss."""
...
def train_model_with_contraction(model, trainloader, valloader, device, num_epochs=10, noise_factor=0.1, lambda_contraction=0.1):
    """Training function with contraction regularization to enforce Banach ideals."""
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-3)
    scheduler = StepLR(optimizer, step_size=5, gamma=0.5)
    best_val_loss = float('inf')
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, _ in trainloader:
            ...
            # Compute the iterative refinement with contraction regularization
        ...
        # Validation (using standard loss, without contraction term)
        model.eval()
        val_loss = 0.0
        with torch.no_grad():
            for inputs, _ in valloader:
...
#############################################
# Main Experiment: Compare Training Modes
#############################################

def main():
    parser = argparse.ArgumentParser(description="Banach integration experiments: comparing models with and without contraction regularization.")
    parser.add_argument("--epochs", type=int, default=10, help="Number of training epochs (default: 10)")
    parser.add_argument("--use_contraction", type=bool, default=False, help="Whether to use contraction regularization (default: False)")
    ...
    # Data preparation: load CIFAR-10 with basic transformation (ToTensor only)
    transform = transforms.ToTensor()
    ...
    # Train model with or without contraction regularization
    if args.use_contraction:
        print("Training with contraction regularization (lambda = {:.3f})".format(args.lambda_contraction))
        model = train_model_with_contraction(model, trainloader, valloader, device, num_epochs=args.epochs, noise_factor=0.1, lambda_contraction=args.lambda_contraction)
    else:
    ...
    mode = "contraction" if args.use_contraction else "standard"
    ...
    results_file = os.path.join(RESULTS_DIR, f"banach_experiment_{mode}_lambda{args.lambda_contraction:.2f}.csv")
